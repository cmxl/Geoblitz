...
|---|---|---|
| `/cities/nearest` hot path, in-process, exact per-thread count | **160 B / request** | `GeoCacheKey.Compute` + span query parsing + validation + `FindNearest` + `CityJson.WriteCities` + `BodyWriter.FlushAsync` |
...
The 160 B is the load-bearing number and is asserted against a 512 B ceiling: it accounts for the
