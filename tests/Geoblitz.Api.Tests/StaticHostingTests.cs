...
public class StaticHostingTests : IDisposable
{
    ...
    {
        _webRoot = Path.Combine(Path.GetTempPath(), "geoblitz-wwwroot-" + Guid.NewGuid());
    ...
    public async Task Root_404s_WhenWebRootDirectoryDoesNotExist()
    {
        ...
        // relying on src/Geoblitz.Api/wwwroot's ambient state on disk, since a developer who
        // already ran the publish script locally would otherwise make this test flaky.
        var missingWebRoot = Path.Combine(Path.GetTempPath(), "geoblitz-missing-wwwroot-" + Guid.NewGuid());
